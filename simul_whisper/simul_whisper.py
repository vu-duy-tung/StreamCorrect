...
def create_whisper_model(cfg: AlignAttConfig):
    """
    Factory function to create the appropriate Whisper model based on model_path.
    
    Returns either PaddedAlignAttWhisper (for OpenAI models) or HuggingFaceWhisper
    (for HuggingFace fine-tuned models).
    
    Args:
        cfg: Configuration object with model_path
    
    Returns:
        Whisper model instance (PaddedAlignAttWhisper or HuggingFaceWhisper)
    """
    # Check if this is a HuggingFace model (format: "username/model-name")
    if "/" in cfg.model_path and not os.path.exists(cfg.model_path):
        # This looks like a HuggingFace model identifier
        from .hf_whisper import HuggingFaceWhisper
        logger.info(f"Detected HuggingFace model format: {cfg.model_path}")
        return HuggingFaceWhisper(cfg)
    else:
        # This is a local file or OpenAI model name
