...
from error_corrector.data import datasets
from error_corrector.data.types import ERROR_CORRECTOR_TEMPLATE
from error_corrector.model import (
    file_utils, 
    corrector_config, 
    corrector_data_proc, 
    corrector_model, 
    corrector_pipeline,
    corrector_processing,
    corrector_tokenizer
)
from error_corrector.training import config_base
from error_corrector.utils import device_helpers
...
class CorrectorModelPack(ModelPack):
    def __init__(self, args: config_base.TrainConfig):
        ...
        self.config = corrector_config.CorrectorConfig(
            ...
            audio_token_index=corrector_tokenizer.get_audio_token_id(
                self.text_tokenizer
            ),
        )

        # Instantiate the model
        self.model: corrector_model.CorrectorModel = corrector_model.CorrectorModel(
            self.config
        )

        self.processor = corrector_processing.CorrectorProcessor(
            self.text_tokenizer
        )
        ...
        self.data_collator = corrector_processing.DataCollatorForSeq2SeqWithAudio(
        ...
        if model_vocab_size != tokenizer_vocab_size + (
            1
            if corrector_tokenizer.get_audio_token_id(self.text_tokenizer) is not None
            else 0
        ):
            logging.warning(
    ...
    def wrap_with_data_proc(self, dataset: datasets.SizedIterableDataset):
        return corrector_data_proc.CorrectorDataproc(
            dataset,
    ...
    def get_pipeline(self):
        return corrector_pipeline.CorrectorPipeline(
...
def create_model_pack(config: config_base.TrainConfig) -> ModelPack:
    ...
    if config.model_type == "error_corrector":
        model_pack = (
            LLMOnlyModelPack(config)
            if config.llm_only_training
            else CorrectorModelPack(config)
        )
    else:
    ...
    if config.model_load_dir and (
        ...
        for path in paths:
            ...
            if isinstance(model_pack.model, corrector_model.CorrectorModel):
