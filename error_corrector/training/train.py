...
# from error_corrector.evaluation import eval
# from error_corrector.inference import infer
from error_corrector.model import file_utils
from error_corrector.data import types
from error_corrector.training import config_base, ddp_utils, model_types
from error_corrector.training import trainer as ultra_trainer
from error_corrector.training.helpers import prefetch_weights
from error_corrector.utils import device_helpers, monkey_patches
...
def prepare_dataset(
    ...
    data_args: datasets.CorrectorDatasetArgs,
    verbose: bool = False,
) -> datasets.SizedIterableDataset:
    """
    Prepare the training dataset.
    
    Currently uses a hardcoded path to sample data. The num_samples is taken from
    data_args.max_samples if specified, otherwise counts lines in the file.
    
    Args:
        train_args: Training configuration
        model_pack: Model pack with data processor
        data_opts: Dataset options (currently ignored - for future use with registry)
        data_args: Dataset arguments including max_samples
        verbose: Whether to print verbose output
    """
    data_weights = [1.0]
    
    dataset_path = "error_corrector/data/sample_custom_data/samples.jsonl"
    
    # Count actual samples in the file if max_samples is -1
    if data_args.max_samples == -1:
        with open(dataset_path, 'r') as f:
            actual_num_samples = sum(1 for _ in f)
        num_samples = actual_num_samples
        if verbose:
            logging.info(f"Dataset file has {actual_num_samples} samples")
    else:
        num_samples = data_args.max_samples
    
    ds_config = types.DatasetConfig(
        name="corrector_ds",
        path=dataset_path,
        batch_size=train_args.batch_size,
        splits=[types.DatasetSplitConfig(
            name="corrector_ds",
    ...
    if verbose:
        logging.info(f"Loading dataset from {ds_config.path} with batch_size={ds_config.batch_size}, num_samples={num_samples}")
    
    data_sets = [
        datasets.GenericDataset(data_args, ds_config)
    ]
    
    # If we're using epochs to train (max_steps < 0), validate the dataset length is appropriate.
...
def train(config: config_base.TrainConfig):
    ...
    logging.basicConfig(level=logging.INFO if is_local_master else logging.ERROR, force=True)
    ...
    requested_val = config.val_dataset_args.max_samples
    if requested_val == -1:
        requested_val = max(1, int(0.1 * total_train_len)) if total_train_len else 0
    val_split_size = min(requested_val, total_train_len)

    # Ensure we keep at least world_size samples for training to avoid
    # any rank getting zero samples in DDP mode.
    min_train_samples = world_size if config.do_train else 1
    if val_split_size > total_train_len - min_train_samples and total_train_len > 0:
        val_split_size = max(total_train_len - min_train_samples, 0)
        if val_split_size == 0 and is_global_master:
            logging.warning(
                f"Dataset too small ({total_train_len} samples) to split for validation "
                f"with {world_size} GPUs. Using all samples for training, no val split."
            )

    val_split = (
        datasets.SliceDataset(base_train_dataset, start=0, length=val_split_size)
        if val_split_size > 0
        else None
    )
    train_dataset = (
        datasets.SliceDataset(base_train_dataset, start=val_split_size)
        if val_split_size > 0
        else base_train_dataset
    )

    # Always use split_batches with DDP when using small datasets to avoid
    # DistributedSampler giving some ranks zero samples.
    # Only rank 0 loads real data, others get EmptyDataset.
    split_batches_flag = True
    
    # With split_batches=True:
    # - per_device_batch_size is the TOTAL batch size (not per-GPU)
    # - The batch is split across GPUs, so each gets batch_size // world_size samples
    # - We need batch_size to be divisible by world_size
    train_len = len(train_dataset)
    
    # Use config.batch_size directly (it's the total batch with split_batches=True)
    # Ensure it's divisible by world_size and doesn't exceed dataset size
    effective_train_batch = min(config.batch_size, train_len)
    # Round down to nearest multiple of world_size
    effective_train_batch = max(world_size, (effective_train_batch // world_size) * world_size)
    
    val_len = val_split_size if val_split_size > 0 else 1
    effective_val_batch = min(config.val_batch_size, val_len)
    effective_val_batch = max(world_size, (effective_val_batch // world_size) * world_size)
    
    if is_global_master:
        logging.info(
            f"Batch size config: batch_size={config.batch_size}, world_size={world_size}, "
            f"effective_train_batch={effective_train_batch} (per-GPU: {effective_train_batch // world_size})"
        )

    if is_global_master:
        val_options = config.get_val_sets()
        if len(val_options) == 0:
            val_options = [config_base.DatasetOptions(name="train_split")]

        for val_opt in val_options:
            if val_split is not None:
                val_datasets[val_opt.name] = val_split
            # If no val_split, don't add anything - trainer will skip evaluation

        logging.info(
            f"Loaded train set length: {total_train_len}, val split: {val_split_size}, remaining train: {len(train_dataset)}"
        )
    else:
        ...
        # Use EmptyDataset since rank 0 handles all data loading with split_batches=True
        train_dataset = datasets.EmptyDataset(len(train_dataset))
        if val_split_size > 0:
            val_options = config.get_val_sets()
            if len(val_options) == 0:
                val_options = [config_base.DatasetOptions(name="train_split")]
            for val_opts in val_options:
                val_datasets[val_opts.name] = datasets.EmptyDataset(val_split_size)
    ...
    trainer = ultra_trainer.UltraTrainer(
        ...
        args=transformers.Seq2SeqTrainingArguments(
            ...
            # With split_batches=True, per_device_batch_size is the TOTAL batch that gets split
            # So we use effective batch sizes computed above to ensure no rank gets zero samples
            per_device_train_batch_size=(
                effective_train_batch if not config.use_dynamic_batch else 1
            ),
            per_device_eval_batch_size=(
                effective_val_batch if not config.use_dynamic_batch else 1
            ),
            dataloader_drop_last=config.dataloader_drop_last,
            accelerator_config={
                "split_batches": split_batches_flag,
            },
