...
class TrainConfig:
    ...
    loss_mask_type: corrector_config.LossMaskType = (
        corrector_config.LossMaskType.LAST_ASSISTANT
    )
    ...
    # LoRA configs
    text_model_lora_config: Optional[corrector_config.LoraConfigSimplified] = None
    audio_model_lora_config: Optional[corrector_config.LoraConfigSimplified] = None
    ...
    # NOTE: Set to False to enable ln_post normalization which ensures audio embeddings
    # have similar scale to text embeddings. This is critical for proper training.
    ...
    # Loss config
    loss_config: Optional[corrector_config.LossConfig] = None
    ...
    def __post_init__(self):
        ...
        if self.val_steps:
            product = world_size * self.val_batch_size
            max_samples = self.val_dataset_args.max_samples
            if max_samples != -1 and max_samples % product != 0:
                adjusted = int(math.ceil(max_samples / product) * product)
                logging.warning(
                    "val_dataset_args.max_samples (%s) is not divisible by world_size*val_batch_size (%s). "
                    "Adjusting to %s.",
                    max_samples,
                    product,
                    adjusted,
                )
                self.val_dataset_args.max_samples = adjusted
