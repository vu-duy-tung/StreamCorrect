...
from error_corrector.model.corrector_config import LossConfig, LossFunction, CorrectorConfig
...
class CorrectorModel(transformers.LlamaPreTrainedModel, GenerationMixin):
    """
    The Corrector model which consists of an audio encoder and a language model.
    ...
    config: CorrectorConfig  # for type hinting
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass for the Corrector model.

        `input_ids` are the tokenized text input. They are embedded by the language model as usual.
    `audio_values` should be the output of an external audio encoder. They are stacked in groups of
    `stack_factor` frames and projected to the language model's embedding space using a few linear layers.
    ...
    audio_token_start_idx: Optional[torch.Tensor] = None,
        ...
        ), "inputs_embeds/audio_values/audio_token_start_idx/audio_token_len/audio_batch_size must be provided."
        assert (
            len(audio_token_start_idx)
            == len(audio_token_len)
            == len(audio_values)
        ), "audio_token_start_idx/audio_token_len/audio_values must have the same batch size."
        assert len(audio_batch_size) == len(
            inputs_embeds
        ), "audio_batch_size and inputs_embeds must have the same batch size."

        audio_features = audio_values.to(inputs_embeds.dtype)
        audio_embeds = self.multi_modal_projector.forward(audio_features)
    ...
    def generate(
        ...
        **kwargs,
        ...
        lm_end = time.time()
        print(f"Play With Mino - LM Infer time: {1000 * (lm_end - lm_start) :.3f} ms")
        return output

    @classmethod
    def _create_multi_modal_projector(
        cls, config: CorrectorConfig
    ) -> "CorrectorProjector":
        projector = CorrectorProjector(config)
...
class CorrectorProjector(nn.Module):
    def __init__(self, config: CorrectorConfig):
        super().__init__()
    ...
    # Corrector (Ultravox-compatible) v0.4.1 and below uses layer_norm after the second linear layer,
...
CorrectorConfig.register_for_auto_class()
CorrectorModel.register_for_auto_class()

transformers.AutoConfig.register("error_corrector", CorrectorConfig)
transformers.AutoModel.register(CorrectorConfig, CorrectorModel)
