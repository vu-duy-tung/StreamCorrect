...
class CorrectorConfig(transformers.PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`CorrectorForConditionalGeneration`]. It is used to instantiate an
    Corrector model according to the specified arguments, defining the model architecture.
    ...
    ```python
    >>> from transformers import CorrectorModel, WhisperConfig, CorrectorConfig, LlamaConfig
    ...
    >>> configuration = CorrectorConfig(audio_config, text_config)

    >>> # Initializing a completely untrained model from the configuration
    >>> model = CorrectorModel(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config

    >>> # Initialize a model from pretrained checkpoints and random projector weights
    >>> config = CorrectorConfig(audio_model_id="openai/whisper-tiny", text_model_id="meta-llama/Llama-2-7b-chat-hf")
    ```"""

    model_type = "error_corrector"
