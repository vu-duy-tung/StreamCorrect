...
class DataCollatorForSeq2SeqWithAudio(transformers.DataCollatorForSeq2Seq):
    # when enabled, the alt_input_ids, alt_attention_mask, and alt_labels fields are used for computing the KL loss in ErorrCorrectorModel
    ...
    def __call__(self, features, *args, **kwargs):
        ...
        if audio_values and len(audio_values) > 0 and len(audio_values[0]) > 0:
            ...
            batch['audio_values'] = torch.from_numpy(np.stack(audio_values, axis=0))
            ...
            batch["audio_token_len"] = batch["audio_token_len"].unsqueeze_(-1)
            batch["audio_token_start_idx"] = batch["audio_token_start_idx"].unsqueeze_(-1)

        return batch


class CorrectorProcessor(transformers.ProcessorMixin):
    """
    Constructs an Corrector processor which wraps an audio processor and a tokenizer into a single processor.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        config: CorrectorConfig = transformers.AutoConfig.from_pretrained(
...
CorrectorProcessor.register_for_auto_class()

transformers.AutoProcessor.register(CorrectorConfig, CorrectorProcessor)